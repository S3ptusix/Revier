...
import { fetchAllResigned, fetchResignedTotals } from "../services/resignedServices";
...
export default function Resigned() {
    ...
    const loadTotals = async () => {
        const { success, message, totals } = await fetchResignedTotals();
        if (success) return setTotals(totals);
        console.error(message);
    }

    const loadTable = async () => {
        const { success, message, applicants, pagination: apiPagination } = await fetchAllResigned({
            search: toSearch,
            companyId,
            page
        });
                ...
                {isLoading ? (
                    ...
                    <div className="p-8 overflow-auto grow">
                        ...
                        <section className="flex items-center justify-between flex-wrap gap-4 mb-8">
                            <div>
                                <p className="text-2xl font-semibold">Resigned Applicants</p>
                                <p className="text-gray-500">View all resigned applicants</p>
                            </div>
                        </section>
                        ...
                        <section className="grid lg:grid-cols-2 gap-4 mb-8">
                            <div className="border border-gray-300 px-4 py-6 rounded-xl">
                                <div className="flex items-center justify-between mb-8">
                                    <p className="font-semibold text-sm">Total Resigned</p>
