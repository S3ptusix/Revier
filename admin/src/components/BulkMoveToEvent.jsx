import { Calendar, MapPin, X, Check, Loader } from "lucide-react";
...
export default function BulkMoveToEvent({
    applicantIds,
    onClose = () => { },
    loadAfter = () => { },
}) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { success, message } = await bulkMoveToEvent(selectedOrientation.id, { applicantIds });
    ...
    return (
        <>
            <ModalBackground>
                <Modal>
                    <ModalHeader
                        title={`Move ${applicantIds.length} Selected ${applicantIds.length > 1 ? 'Applicants' : 'Applicant'} to Event`}
                        subTitle={`Select an event for ${applicantIds.length} ${applicantIds.length > 1 ? 'Applicants' : 'Applicant'}`}
                        onClose={onClose}
                    />
                    ...
                    `}
                        >
                            {loading ? "Processing..." : "Move to Event"}
                        </button>
                    </div>
