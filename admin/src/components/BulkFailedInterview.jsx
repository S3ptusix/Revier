...
import { bulkFailedInterview } from "../services/interviewServices";
import ItemSelector from "./ui/ItemSelector";

export default function BulkFailedInterview({
    applicantIds,
    loadAfter = () => { },
    onClose = () => { },
}) {
    ...
    const confirmFailedInterview = async () => {
        try {
            setIsLoading(true);
            const { success, message } = await bulkFailedInterview({ ...formData, applicantIds });
